...
- Keep the installation-optional public `@hraness/ui` peer range at `>=0.4.0 <0.5.0` and the development installation pinned to `github:hraness/ui#v0.4.0`. Stylesheet and React consumers install the peer explicitly; the framework-neutral root and syntax highlighter do not require it. Portable controls, React Aria behavior, overlays, fields, collections, navigation primitives, and low-level surfaces belong there.
