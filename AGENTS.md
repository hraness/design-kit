...
- `portfolio-inventory.json` declares the public package, publication, and exact `@hraness/ui` runtime edge.
- `scripts/`, package configuration, and workflows build, smoke-test, verify, and release the standalone package.

# Guidelines

- Use Bun 1.3.14. Run `bun run check` before handing off a change.
- Keep `@hraness/ui` pinned to `github:hraness/ui#v0.4.0`. Portable controls, React Aria behavior, overlays, fields, collections, navigation primitives, and low-level surfaces belong there.
