...
- Keep every handwritten component rule inside `components.hraness-design-kit.legacy` and compiled recipes inside `components.hraness-design-kit.priority1`, `priority2`, and `priority3`. The complete stylesheet keeps `base` below `components` and freezes the exact UI legacy-to-priority3 order before the design-kit legacy-to-priority3 order. The narrow component entry must preserve the same local order, and package gates must structurally reject bare or unlayered rules.
