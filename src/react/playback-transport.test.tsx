...
import { ToggleButton } from "@hraness/ui";
...
test("the transport composes the portable large icon command and owns wrapping", async () => {
  const components = await Bun.file(
    new URL("../components.css", import.meta.url),
  ).text();

  expect(components.match(
    /\.hraness-design-playback-transport\s*\{(?<body>[^}]*)\}/u,
