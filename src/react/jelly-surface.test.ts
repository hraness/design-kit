...
test("Jelly capture handlers preserve consumers and honor cancellation", () => {
  type CaptureEvent = { defaultPrevented: boolean };
  const calls: string[] = [];
  const allowed: CaptureEvent = { defaultPrevented: false };
  composeJellyCapture<CaptureEvent>(
    () => calls.push("consumer"),
    () => calls.push("internal"),
  )(allowed);
  ...
  calls.length = 0;
  const cancelled: CaptureEvent = { defaultPrevented: false };
  composeJellyCapture<CaptureEvent>(
    (event) => {
      calls.push("consumer");
